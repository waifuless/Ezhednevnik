...
void Calendar::printMonth(int monthNumber, int yearNumber) {
    ...
    time(&now); //принимаем текущее время, в котором изменим месяц и год
    ...
    for (i = 1; i < 32; i++) 
    {
        ...
        timeinfo->tm_mday = i;
        mktime(timeinfo);//устанавливается информация о дне(день недели) 

        if (timeinfo->tm_mon == monthNumber) //цикл завершится досрочно, если мы перешли на следующий месяц
            break;
